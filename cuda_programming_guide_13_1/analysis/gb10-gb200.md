...
6. [Networking & Multi-Node Connectivity](#networking--multi-node-connectivity)
...
| Architecture | Blackwell (SM 12.1) | Blackwell (SM 10.0) |
...
| Memory Bandwidth (GPU) | ~301 GB/s | 576 TB/s (8 TB/s per GPU × 72) |
...
**Key Difference:** The GB200 NVL72 uses high-bandwidth HBM3e memory delivering 576 TB/s total GPU bandwidth compared to the GB10's LPDDR5x at ~301 GB/s. This massive 1,914x bandwidth advantage enables the NVL72 to train trillion-parameter models with massive batch sizes where memory access is the primary bottleneck.
...
The GB200 NVL72's HBM3e provides the bandwidth necessary to keep 10,368 SMs fully utilized across 72 GPUs, while the GB10's LPDDR5x at ~301 GB/s is matched to its 48 SMs. The 1,914x bandwidth difference (576 TB/s vs 301 GB/s) reflects the extreme scaling from desktop to datacenter. The 130 TB/s NVLink domain enables near-instantaneous model parallelism across all GPUs.
...
The GB200 NVL72's 576 TB/s aggregate memory bandwidth provides 1,914x advantage over GB10's 301 GB/s for memory-bound operations like attention mechanisms in transformers. The 130 TB/s NVLink domain enables efficient attention across model-parallel splits, making trillion-parameter models practical.
...
- **Memory Bandwidth Advantage**: 576 TB/s vs 339 TB/s aggregate (1.7x) - critical bottleneck for large models
...
- **Memory BW**: 301 GB/s vs 576 TB/s (1,914x)
...
- GB200 NVL72 uses compute capability 10.0 (Blackwell family) with 228 KB shared memory per SM
